...
export const usersTable = sqliteTable(
  "users",
  {
    ...
    onboardingCurrentStep: text("onboarding_current_step")
      .notNull()
      .default("welcome"),
    ...
    walletIsGenerated: integer("wallet_is_generated", {
      mode: "boolean",
    }).default(false),
    ...
    security2faEnabled: integer("security_2fa_enabled", {
      mode: "boolean",
    }).default(false),
    security2faEnabledAt: text("security_2fa_enabled_at"),

    // Preferences
    preferencesNews: integer("preferences_news", { mode: "boolean" }).default(
      true,
    ),
    preferencesOrderFills: integer("preferences_order_fills", {
      mode: "boolean",
    }).default(true),
    preferencesRiskAlerts: integer("preferences_risk_alerts", {
      mode: "boolean",
    }).default(true),
    preferencesStatements: integer("preferences_statements", {
      mode: "boolean",
    }).default(true),
    preferencesTheme: text("preferences_theme").default("dark"),
    preferencesDefaultQuote: text("preferences_default_quote").default("USDC"),
    preferencesHintsEnabled: integer("preferences_hints_enabled", {
      mode: "boolean",
    }).default(true),
  ...
  (table) => ({
    ...
    onboardingStepIdx: index("idx_users_onboarding_step").on(
      table.onboardingCurrentStep,
    ),
    walletPublicKeyIdx: index("idx_users_wallet_public_key").on(
      table.walletPublicKey,
    ),
  }),
);
